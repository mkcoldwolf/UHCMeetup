package mitw.meetup.border.worldborder;
...
public class BorderCheckTask implements Runnable {
    ...
    public static Location checkPlayer(Player player, Location targetLoc, boolean returnLocationOnly, boolean notify) {
        ...
        // tag this player as being handled so we can't getInstance stuck in a loop due to Bukkit currently sometimes repeatedly providing incorrect location through teleport event
    ...
    private static void setPassengerDelayed(final Entity vehicle, final Player player, final String playerName, long delay) {
        Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(UHCMeetup.getInstance(), new Runnable() {
            @Override
            public void run() {
